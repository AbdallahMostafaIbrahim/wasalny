...
Map *MainWindow::getMap() { return &map; }
...
void MainWindow::createNode(int x, int y, QString city) {
  ...
  scene->addItem(node);
  connect(node, &Node::positionChanged, this,
          [this, city](const QPointF &newPos) {
            map.setCityCoordinate(city.toStdString(), newPos.x() + 20 / 2,
                                  newPos.y() + 20 / 2);
            drawEdges();
            scene->update();
          });
...
void MainWindow::drawEdges() {
  ...
  for (const auto &city : map.getCities()) {
    ...
    for (const auto &edge : map.getEdges(city)) {
      ...
      Edge *e =
          new Edge(QPointF(fromCoord.x, fromCoord.y),
                   QPointF(toCoord.x, toCoord.y), QString::fromStdString(city),
                   QString::fromStdString(edge.first), edge.second);
...
void MainWindow::on_searchButton_clicked() {
  ...
  for (size_t i = 0; i < path.size(); ++i) {
    if (i != path.size() - 1) {
      for (auto &edge : edges) {
        if (path[i] == edge->getFromCity().toStdString() &&
            path[i + 1] == edge->getToCity().toStdString()) {
          edge->focusIn();
          break;
        }
      }
...
void MainWindow::on_deleteEdgeButton_clicked() {
  map.deleteEdge(currentEdge->getFromCity().toStdString(),
                 currentEdge->getToCity().toStdString());
  uiState = HOME_UI;
  ui->stackedWidget->setCurrentIndex(HOME_UI);
  drawEdges();
}
...
void MainWindow::on_updateDistanceButton_clicked() {
  ...
  map.addEdge(currentEdge->getFromCity().toStdString(),
              currentEdge->getToCity().toStdString(), distance);
